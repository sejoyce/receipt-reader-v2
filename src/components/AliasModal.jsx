...
// Shown when a receipt contains an abbreviation we don't recognize.
// User can map it to an existing product, or create a new one.
  ...
  async function handleCreate() {
    ...
    try {
      const id = await createProduct({
        ...
        aliases: [unknownItem.description],
      })
    ...
    <div className="modal-overlay">
      <div className="modal">
        ...
        <p style={{ color: 'var(--ink-light)', fontSize: '0.9rem', marginBottom: 20 }}>
          We found <strong style={{ fontFamily: 'monospace', background: 'var(--cream-dark)', padding: '2px 6px', borderRadius: 4 }}>{unknownItem.description}</strong> on your receipt for <strong>${unknownItem.price?.toFixed(2)}</strong>.
          Map it to a product so we can track its price over time.
        </p>
        ...
        {mode === 'search' && (
          <>
            ...
            <div style={{ maxHeight: 240, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 6 }}>
              {filtered.length === 0 && (
                <p style={{ color: 'var(--ink-faint)', fontSize: '0.85rem', textAlign: 'center', padding: '20px 0' }}>
                  No products found. Try creating a new one.
                </p>
              )}
              {filtered.map(p => (
                <button
        ...
        {mode === 'create' && (
          <>
            ...
            <div className="form-group">
              ...
              <input
                className="form-input"
                placeholder="e.g. Dairy, Produce, Bread"
                value={newCategory}
                onChange={e => setNewCategory(e.target.value)}
              />
            </div>
            <p style={{ fontSize: '0.8rem', color: 'var(--ink-faint)' }}>
              "{unknownItem.description}" will be saved as an alias for this product.
            </p>
          </>
        )}
