...
export default function ComparePrices() {
  ...
  useEffect(() => {
    ...
    getPriceHistoryForProduct(selectedProduct.id).then(entries => {
      ...
      // Build chart data: one point per date, one series per store
      const byDate = {}
  ...
  const storeStats = stores.map(store => {
    ...
    return {
      ...
      min: Math.min(...prices),
      max: Math.max(...prices),
      latest: entries[entries.length - 1]?.price,
    ...
    <div className="animate-fade">
      ...
      {!selectedProduct && (
        <div className="empty-state">
          ...
          <p>Choose a product to see its price history across stores.</p>
        </div>
      )}
      ...
      {selectedProduct && !loading && history.length === 0 && (
        <div className="empty-state">
          ...
          <p>Upload receipts containing this product to see price comparisons.</p>
        </div>
      )}
