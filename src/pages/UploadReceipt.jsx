...
export default function UploadReceipt() {
  ...
  function handleAliasResolved(resolvedItem) {
    setItems(prev =>
      prev.map(it => it.description === resolvedItem.description ? resolvedItem : it)
    )
    advanceQueue()
  }
  ...
  async function handleSave() {
    if (!storeName.trim()) {
      toast('Please enter a store name', 'error')
      return
    }
    setStep('saving')
    try {
        ...
        {['Upload', 'Process', 'Review', 'Save'].map((label, i) => {
          ...
          return (
            <div key={label} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span style={{ fontSize: '0.8rem', color: active ? 'var(--ink)' : 'var(--ink-faint)', fontWeight: active ? 600 : 400 }}>
                {label}
              </span>
        ...
        <div className="grid-2" style={{ gap: 24, alignItems: 'start' }}>
          <div>
            <div
              ...
              style={{
                border: `2px dashed ${imageFile ? 'var(--green)' : 'var(--border)'}`,
                borderRadius: 'var(--radius-lg)',
                padding: '40px 24px',
                textAlign: 'center',
                cursor: 'pointer',
                background: imageFile ? 'var(--green-pale)' : 'var(--cream)',
                transition: 'all 0.2s',
                marginBottom: 16,
              }}
            >
              {imagePreview ? (
                ...
                <>
                  <UploadCloud size={36} color="var(--ink-faint)" style={{ marginBottom: 12 }} />
                  <p style={{ fontWeight: 600, marginBottom: 4 }}>Drop receipt image here</p>
                  <p style={{ fontSize: '0.8rem', color: 'var(--ink-faint)' }}>or click to browse · JPG, PNG, WEBP</p>
                </>
              )}
            </div>
          ...
          <div>
            <div className="card">
              ...
              <div className="form-group">
                ...
                <input className="form-input" placeholder="e.g. Trader Joe's" value={storeName} onChange={e => setStoreName(e.target.value)} />
              </div>
              <div className="form-group">
        ...
        <div>
          {currentUnknown && (
            <AliasModal
              ...
              onResolved={handleAliasResolved}
              onSkip={advanceQueue}
            />
          )}
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              <button className="btn btn-primary" onClick={handleSave} disabled={items.filter(i => i.productId).length === 0}>
                Save Receipt
              </button>
            </div>
          </div>
