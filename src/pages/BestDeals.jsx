...
export default function BestDeals() {
  ...
  const [sortBy, setSortBy] = useState('name') // 'name' | 'savings' | 'price'

  useEffect(() => {
    async function load() {
      const [dealData, history] = await Promise.all([
        getBestDeals(),
        getAllPriceHistory(),
      ])

      // Enrich with price spread (max - min) and last seen date
  ...
  return (
    <div className="animate-fade">
      <div className="page-header">
        <h2>Best Deals</h2>
        <p>The lowest price we've ever seen for each product, and where to buy it.</p>
      </div>
          ...
          <div key={deal.productId} className="card" style={{ position: 'relative', overflow: 'hidden' }}>
            ...
            {deal.spread > 0 && (
              <div style={{ marginTop: 12, paddingTop: 12, borderTop: '1px solid var(--cream-dark)', fontSize: '0.78rem', color: 'var(--ink-light)' }}>
                Range: ${Math.min(...(deal.entries || []).map(e => e.price)).toFixed(2)} – ${Math.max(...(deal.entries || []).map(e => e.price)).toFixed(2)}
              </div>
            )}
          </div>
        ))}
